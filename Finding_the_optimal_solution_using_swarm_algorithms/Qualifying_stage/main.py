...
if __name__ == "__main__":
    ...
    flight_mission = FlightMissionRunner(MAP_COVERAGE_POINTS)
    ...
    first_point = flight_mission.get_next_point()
    x, y = first_point
    pioneer.go_to_local_point(x=x, y=y, z=flight_height, yaw=0)


    while not flight_mission.is_complete():
        ...
        if pioneer.check_hover_complete():
            next_point = flight_mission.get_next_point()
            if next_point:
                x, y = next_point
                pioneer.go_to_local_point(x=x, y=y, z=flight_height, yaw=0)
    ...
    planner = FlightMissionPathPlanner(all_avg_coords, pioneer)
    coordinates_plan = planner.get_coordinates_plan() 

    marker_averager = ArucoMarkerAverager() 

    flight_mission = FlightMissionRunner(coordinates_plan)
    first_point = flight_mission.get_next_point()
    x, y = first_point
    pioneer.go_to_local_point(x=x, y=y, z=flight_height, yaw=0)

    hover_duration_over_marker = 2
    landing_pause_time = 0.2
    ...
    while not flight_mission.is_complete():
        ...
        if pioneer.check_hover_complete():
            if flight_mission.has_more_points():
